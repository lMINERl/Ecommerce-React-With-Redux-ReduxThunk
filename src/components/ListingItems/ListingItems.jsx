...
const ListingItems = props => {
  ...
  const pageNumration = () => {
    let pageNum = [];
    let numOfPages =
      props.products.length % props.max_items === 0
        ? Math.floor(props.products.length / props.max_items)
        : Math.floor(props.products.length / props.max_items) + 1;
    for (let index = 0; index < numOfPages; index++) {
      if (index + 1 === props.page) {
        pageNum.push(
          <div key={index} className="paging__number active">
            {index + 1}
          </div>
        );
      } else {
        pageNum.push(
          <div key={index} className="paging__number">
            {index + 1}
          </div>
        );
      }
  ...
  const productsList = props.products.map((p, index) => {
    if (
      index >= (props.page - 1) * props.max_items &&
      index < props.page * props.max_items
    ) {
      return (
  ...
  listingItems = (
    <section className="item-listing">
      ...
      <div className="paging">
        <div
          onClick={() => {
            props.pageChanedHndlr(props.page - 1);
          }}
