...
class Form extends Component {
  constructor(props) {
    ...
    this.state = {
      product: {
        name: "",
        img:
          "https://obamawhitehouse.archives.gov/sites/default/files/image/image_file/50-50-page-header.jpg",
        description: "",
        category: "",
        addedBy: "",
        price: "",
  ...
  paymentChanged = event => {
    ...
    if (event.target.checked) {
      ...
      const index = newProduct.paymentTypes.findIndex(
        p => p === event.target.name
      );
  ...
  tagAdded = event => {
    if (event.key === "Enter") {
      let newTags =
        this.state.product.tags.length > 0 ? [...this.state.product.tags] : [];
  ...
  shouldComponentUpdate = (prevProp, prevState) => {
    ...
    const isSame =
      dataComp(this.props.data, prevProp.data) &&
    ...
    if (this.state.allowUser) {
      ...
      if (this.state.product.tags && this.state.product.tags.length > 0) {
        tags = this.state.product.tags.map((t, index) => {
          return (
            <div key={index} className="taged-textbox__tag">
              <span>{t}</span>
              <span className="taged-textbox__remove">
                <i
                  onClick={this.tagRemoved.bind(this, index)}
        ...
        tags = (
          <>
            {tags}{" "}
            <div
              className="taged-textbox__clear"
              onClick={() => {
                this.setState({ product: { ...this.state.product, tags: [] } });
              }}
            >
              <span>
      ...
      let btn = this.props.match.params.id ? (
        <button
          onClick={this.userEdited}
          type="submit"
                  ...
                  <div className="form-controls">
                    ...
                    <div className="add-product__discount">
                        ...
                        <div className="form-group__radios">
                          <div className="form-group__radio">
                            <input
                              ...
                              name="isOnSale"
                              value="1"
                              checked={this.state.product.isOnSale}
                      ...
                      <div className="form-group">
                        ...
                        <input
                          disabled={!this.state.product.isOnSale}
                          className="form-control"
                    ...
                    <div className="taged-textbox form-group">
                      ...
                      <div className="taged-textbox__data">
                        <div className="taged-textbox__tags">{tags}</div>
                      </div>
                      <input
                        className="taged-textbox__textbox form-control"
                        type="text"
                        onKeyUp={this.tagAdded}
                      />
                    </div>
                    <div className="add-product__actions">
                      <button onClick={this.cancel} className="btn btn--gray">
                        Cancel
                      </button>
                      {btn}
                    </div>
...
const mapDispatchToProps = dispatch => {
  return {
    onAdd: (value, token) => {
      dispatch(addDispatch({ value, token }));
    },
    onSelect: id => {
