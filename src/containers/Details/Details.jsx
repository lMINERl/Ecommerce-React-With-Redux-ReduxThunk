...
import { getProductDispatch, DispatchChangeCache } from '../../store/actions/actions';
...
class Details extends React.Component {
  ...
  shouldComponentUpdate = (prevProps, prevState) => {
    ...
    const isSame = dataComp(prevProps.product, this.props.product) && dataComp(this.state, prevState);
    return !isSame;
  }
  render() {

    // console.log(`render Details and allow user is ${this.state.allowUser}`);
    let details = (<><h1>Waiting For authurization</h1></>);
    if (this.state.allowUser) {

      let price = (<span>${this.props.product.price}</span>);
      if (this.props.product.isOnSale) {
        price = (<><span style={{ color: 'lightgray', textDecoration: 'line-through' }}>${this.props.product.price}</span>&nbsp;<span>${(parseInt(this.props.product.price) - parseInt(this.props.product.discount)).toString()}</span>
        </>);
      }
            ...
            <section className="product-details__main">
              <div className="slider">
                <div className="slider__items">
                  <div className="slider__item active" style={{ backgroundImage: `url(${this.props.product.image})` }}></div>
                </div>
              </div>
...
const mapDispatchToProps = dispatch => {
  return {
    onLoad: (id) => { dispatch(getProductDispatch(id)) },
