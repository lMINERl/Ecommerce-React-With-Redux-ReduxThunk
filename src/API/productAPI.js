...
const productAPI = {
    ...
    getProductById(productId) {
        ...
        return axios.get(`${baseURL}/products/${productId}`, { headers: { authorization: token } });
    },
    // returns an Array of products value (v) where as (v) contains direct list of products
    getAllProducts() {
        const token = accessToken.authorization();
        // debugger;
        return axios.get(`${baseURL}/products/`, { headers: { authorization: token } });
    },
    // returns  product value of (v) where as (v) contains product
    deleteProduct(productId) {
        const token = accessToken.authorization();
        return axios.delete(`${baseURL}/products/${productId}`, { headers: { authorization: token } });
    },
    addProduct(product) {
        const token = accessToken.authorization();
